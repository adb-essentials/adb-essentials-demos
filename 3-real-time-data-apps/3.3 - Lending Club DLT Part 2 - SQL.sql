...
CREATE INCREMENTAL LIVE TABLE lendingclub_payments_silver(
...
AS SELECT *
FROM STREAM(LIVE.lendingclub_bronze)
...
CREATE LIVE TABLE lendingclub_payments_gold_complete(
...
FROM LIVE.lendingclub_payments_silver
...
CREATE INCREMENTAL LIVE TABLE lendingclub_payments_gold_incr_agg(
...
AS SELECT
  ...
  sum(payment_amnt) as total_payment,
  count(*) as total_payments
FROM STREAM(LIVE.lendingclub_payments_silver)
GROUP BY eventdate, addr_state
...
CREATE INCREMENTAL LIVE TABLE loans_gold_incr_agg_partitioned(
  CONSTRAINT valid_payment EXPECT (total_payment > 0) ON VIOLATION FAIL UPDATE
)
PARTITIONED BY ( eventdate )
COMMENT "partitioned states with incremental aggregation"
AS SELECT
  to_date(timestamp) as eventdate,
  addr_state,
  sum(funded_amnt) as total_funded,
...
CREATE INCREMENTAL LIVE TABLE lendingclub_payments_gold_window
